...
async def get_seedance_video(prompt: str, model: Literal['seedance_pro', 'seedance_lite'], duration: Literal[5, 10], sizes: Literal["16:9", "9:16"], image: str | None = None) -> str:
    data = {
        'prompt': prompt,
        'duration': duration,
        'aspect_ratio': sizes,
    ...
    try:
        output = await client.predictions.async_create(
            model=f"bytedance/{models[model]}",
            input=data
        )
